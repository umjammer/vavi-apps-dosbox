...
public class BitBlt extends WinAPI {
    ...
    static public void StretchBlt2D(Graphics2D graphics, int nXOriginDest, int nYOriginDest, int nWidthDest, int nHeightDest, BufferedImage src, int nXOriginSrc, int nYOriginSrc, int nWidthSrc, int nHeightSrc, int dwRop) {
        // every raster operation other than a straight copy is drawn as one: the combining ones
        // - the inversions a program uses to highlight with - would each need their own pass over
        // the pixels, and what they are for is how the window looks, not what it does
        if (dwRop != SRCCOPY) {
            warn("StretchBlt drawing raster operation 0x" + Integer.toHexString(dwRop) + " as a copy");
        }
