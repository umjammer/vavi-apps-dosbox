...
public class Dos_programs {
    ...
    private static final Logger logger = System.getLogger(Dos_programs.class.getName());
        ...
        public void run() {
            ...
            while (i < cmd.getCount()) {
                if ((temp_line = cmd.findCommand(i + 1)) != null) {
                    ...
                    writeOut(Msg.get("PROGRAM_BOOT_IMAGE_OPEN"), temp_line);
        ...
        public void run() {
            ...
            if (type.equals("floppy") || type.equals("hdd") || type.equals("iso") || type.equals("zip")) {
                ...
                while ((temp_line = cmd.findCommand(paths.size() + 2)) != null && !temp_line.isEmpty()) {
                    ...
                    if (!f.exists()) {
                        ...
                        if (f.exists()) {
                            ...
                            if (!(Dos_files.Drives[dummy.value] instanceof Drive_local ldp)) {
                ...
                switch (fstype) {
                    case "fat" -> {
                        if (imgsizedetect) {
                            ...
                            try {
                                ...
                                if (yet_detected)
                                    writeOut(Msg.get("PROGRAM_IMGMOUNT_AUTODET_VALUES").formatted(sizes[0], sizes[1], sizes[2], sizes[3]));
                                else {
                                    writeOut(Msg.get("PROGRAM_IMGMOUNT_INVALID_GEOMETRY"));
                                    return;
                                }
                    ...
                    case "zip" -> newdrive = new Drive_zip(temp_line);
                    default -> {
                        FileIO newDisk = null;
                        try {
            ...
            switch (fstype) {
                ...
                case "iso" -> {
                    ...
                    if (ide_index.value >= 0) {
                        try {
                            IDE.IDE_Attach(true, ide_index.value, ide_slave.value, FileIOFactory.open(paths.getFirst(), FileIOFactory.MODE_READ), 0, 0, 0);
                    ...
                    List<Dos_Drive> isoDisks = new ArrayList<>();
                    int i;
                    int ct;
                    for (i = 0; i < paths.size(); i++) {
                        IntRef error = new IntRef(-1);
                        Dos_Drive newDrive = new Drive_iso(drive, paths.get(i), mediaid, error);
...
//                        }
                    ...
                    // Update DriveManager
                    for (ct = 0; ct < isoDisks.size(); ct++) {
                        DriveManager.AppendDisk(drive - 'A', isoDisks.get(ct));
                    }
                    ...
                    writeOut(Msg.get("MSCDEX_SUCCESS"));
                    StringBuilder tmp = new StringBuilder(paths.getFirst());
                    for (i = 1; i < paths.size(); i++) {
                        tmp.append("; ").append(paths.get(i));
                    }
                    writeOut(Msg.get("PROGRAM_MOUNT_STATUS_2"), drive, tmp.toString());
                }
                case "zip" -> {
                    if (Dos_files.Drives[drive - 'A'] != null) {
                        writeOut(Msg.get("PROGRAM_IMGMOUNT_ALREADY_MOUNTED"));
    ...
    public static void DOS_SetupPrograms() {
        ...
        Msg.add("PROGRAM_IMGMOUNT_INVALID_IMAGE", """
                ...
                """);
        Msg.add("PROGRAM_IMGMOUNT_INVALID_GEOMETRY", """
