...
public class VGA_dac {
    ...
    private static final Logger LOG_VGAGFX = System.getLogger("LOG_VGAGFX");
    ...
    */
    ...
    static private void VGA_DAC_SendColor( /*Bitu*/int index, /*Bitu*/int src ) {
        /*Bitu*/short red = VGA.vga.dac.rgb[src].red;
        /*Bitu*/short green = VGA.vga.dac.rgb[src].green;
        /*Bitu*/short blue = VGA.vga.dac.rgb[src].blue;
    ...
    static private final IoHandler.IO_WriteHandler write_p3c6 = (port, val, iolen) -> {
        if ( VGA.vga.dac.pel_mask != val ) {
            LOG_VGAMISC.log(Level.DEBUG, "VGA:DCA:Pel Mask set to "+Integer.toString(val,16));
...
//		VGA.vga.dac.read_index = VGA.vga.dac.write_index - 1;//disabled as it breaks Wari
            ...
            break;
        default:
            LOG_VGAGFX.log(Level.DEBUG, "VGA:DAC:Illegal Pel Index");			//If this can actually happen that will be the day
            break;
        }
    };
...
//		VGA.vga.dac.write_index=VGA.vga.dac.read_index+1;//disabled as it breaks wari
            break;
        default:
            LOG_VGAMISC.log(Level.DEBUG, "VGA:DAC:Illegal Pel Index");			//If this can actually happen that will be the day
            ret=0;
            break;
        }
        return ret;
