...
public class Pic extends Module_base {
    ...
    static private class PIC_Controller {
        ...
        void check_for_irq() {
            /*Bit8u*/
            int possible_irq = (irr & imrr) & isrr;
            if (possible_irq != 0) {
                /*Bit8u*/
                int a_irq = special ? 8 : active_irq;
    ...
    static private final IoHandler.IO_WriteHandler write_data = (port, val, iolen) -> {
        ...
        switch (pic.icw_index) {
            ...
            */
                ...
                if ((val & 0x01) == 0)
                    throw new IllegalStateException("PIC:ICW4: " + Integer.toString(val, 16) + ", 8085 mode not handled");
                if ((val & 0x10) != 0)
                    logger.log(Level.DEBUG, "PIC:ICW4: " + Integer.toString(val, 16) + ", special fully-nested mode not handled");
