...
//       anything and actually works in WHATVGA just fine.
    ...
    static private final VGA.tWritePort write_p3cf_pvga1a = (reg, val, iolen) -> {
        ...
        switch (reg) {
            ...
            case 0x0f:
                ...
                break;
            default:
                if (LOG_VGAMISC.isLoggable(Level.DEBUG))
                    LOG_VGAMISC.log(Level.DEBUG, "VGA:GFX:PVGA1A:Write to illegal index " + Integer.toString(reg, 16));
                break;
        }
    };
    ...
    static private final VGA.tReadPort read_p3cf_pvga1a = (reg, iolen) -> {
        ...
        switch (reg) {
            ...
            default:
                if (LOG_VGAMISC.isLoggable(Level.DEBUG))
                    LOG_VGAMISC.log(Level.DEBUG, "VGA:GFX:PVGA1A:Read from illegal index " + Integer.toString(reg, 16));
                break;
        }
        return 0x0;
    };
