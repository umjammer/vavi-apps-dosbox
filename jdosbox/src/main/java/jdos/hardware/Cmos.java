...
public class Cmos extends Module_base {
    ...
    private static final Logger LOG_PIT = System.getLogger("LOG_PIT");
    ...
    static private void cmos_checktimer() {
        ...
        LOG_PIT.log(Level.DEBUG, "RTC Timer at "+ StringHelper.format(1000.0/cmos.timer.delay, 2)+" hz");
    ...
    static private final IoHandler.IO_WriteHandler cmos_writereg = (port, val, iolen) -> {
        ...
        case 0x05:		/* Hours Alarm */
            LOG_BIOS.log(Level.DEBUG, "CMOS:Trying to set alarm");
            cmos.regs[cmos.reg]=(byte)val;
            break;
        case 0x0a:		/* Status reg A */
            cmos.regs[cmos.reg]=(byte)(val & 0x7f);
            if ((val & 0x70)!=0x20) LOG_BIOS.log(Level.ERROR, "CMOS Illegal 22 stage divider value");
        ...
        case 0x0b:		/* Status reg B */
            ...
            if ((val&0x10)!=0) LOG_BIOS.log(Level.ERROR, "CMOS:Updated ended interrupt not supported yet");
            cmos_checktimer();
            break;
        ...
        default:
            cmos.regs[cmos.reg]=(byte)(val & 0x7f);
            LOG_BIOS.log(Level.ERROR, "CMOS:WRite to unhandled register "+Integer.toString(cmos.reg,16));
        }
    };
    ...
    static private final IoHandler.IO_ReadHandler cmos_readreg = (port, iolen) -> {
        if (cmos.reg>0x3f) {
            LOG_BIOS.log(Level.ERROR, "CMOS:Read from illegal register "+Integer.toString(cmos.reg,16));
            return 0xff;
        }
        ...
        case 0x29:
            if (Bios.boot)
                return cmos.regs[cmos.reg] & 0xFF;
            if(Bios_disk.imageDiskList[3] != null) return 0xc0 | (((Bios_disk.imageDiskList[3].heads) > 8)?1:0 << 3);
            return 0;
        case 0x2a:
            if (Bios.boot)
                return cmos.regs[cmos.reg] & 0xFF;
    ...
    //		LOG_BIOS.log(Level.DEBUG, "CMOS:Read from reg %X : %04X",cmos.reg,cmos.regs[cmos.reg]);
            return cmos.regs[cmos.reg] & 0xFF;
        default:
            LOG_BIOS.log(Level.DEBUG, "CMOS:Read from reg "+Integer.toString(cmos.reg,16));
            return cmos.regs[cmos.reg];
        }
    };
