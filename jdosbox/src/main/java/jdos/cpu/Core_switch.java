...
//                        Record.op(b.opCode);
//                        if (count>0) {
//                            logger.log(Level.DEBUG,String.format("%d %06x:%08x %3s %-8s EAX=%08x ECX=%08x EDX=%08x EBX=%08x ESP=%08x EBP=%08x ESI=%08x EDI=%08x FLAGS=%04x", count, CPU_Regs.reg_csVal.dword, CPU_Regs.reg_eip, Integer.toHexString(b.opCode), (Core_normal.desc.length > b.opCode ? " " + Core_normal.desc[b.opCode] : ""), CPU_Regs.reg_eax.dword, CPU_Regs.reg_ecx.dword, CPU_Regs.reg_edx.dword, CPU_Regs.reg_ebx.dword, CPU_Regs.reg_esp.dword, CPU_Regs.reg_ebp.dword, CPU_Regs.reg_esi.dword, CPU_Regs.reg_edi.dword, CPU_Regs.flags));
//                            count++;
//                            if ((count%1000)==0) {
//                                int ii=0;
//                            }
