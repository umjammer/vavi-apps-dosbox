...
public class Main extends MainBase {
    ...
    static public void GFX_Events() {
        while (!events.isEmpty()) {
            ...
            events.removeFirst();
            switch (event) {
                case null -> throw new ShutdownException();
                case KeyEvent keyEvent -> handle(keyEvent);
                case FocusChangeEvent focusChangeEvent -> handle(focusChangeEvent);
                case MouseEvent mouseEvent -> handle(mouseEvent);
                default -> {
                }
            }
        }
    ...
    static private void handle(MouseEvent event) {
        if (event.getID() == MouseEvent.MOUSE_MOVED || event.getID() == MouseEvent.MOUSE_DRAGGED) {
            ...
            if (mouse_locked || !mouse_autoenable) {
                if (event instanceof MouseEvent2 event2) {
    ...
    static public void GFX_EndUpdate() {
        if (pitch != 0) {
            if (startupTime != 0) {
                logger.log(Level.DEBUG, "Startup time: " + (System.currentTimeMillis() - startupTime) + "ms");
                startupTime = 0;
            }
            long startTime = 0;
